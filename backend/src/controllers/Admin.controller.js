...
const options = {
  ...
  const registerAdmin = asyncHandler(async (req, res) => {
    ...
    const adminCreated = await Admin.findById(newAdmin._id).select("-password -refreshToken");
    if (!adminCreated) {
  ...
  const updateAdminDetails = asyncHandler(async (req, res) => {
    ...
    return res
      .status(200)
      .json(new ApiSuccess(200, updatedAdmin, "Successfully updated admin details"));
  });
  export {registerAdmin,loginAdmin,logoutAdmin,getCurrentAdmin,updateAdminDetails,changeAdminPassword,refreshAdminAccessToken}
