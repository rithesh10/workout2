...
export default function DietPlan() {
  ...
  return (
    <div className="min-h-screen w-screen overflow-x-hidden bg-gray-50 flex flex-col items-center justify-center py-6 px-4 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-8 text-center">Create Your Diet Plan</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border bg-white rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                required
              />
            </div>
            <div>
  ...
  <div className="bg-white rounded-lg shadow-lg mt-12 w-full max-w-4xl p-6">
    <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Diet Plan for {formData.FitnessGoal}</h2>
    <div className="space-y-8">
      {dietPlan.dailyDiet.map((meal, index) => (
        <div className="bg-white p-5 rounded-lg shadow-sm" key={index}>
          <h3 className="text-xl font-semibold text-blue-600">{meal.typeOfMeal}</h3>
          <ul className="mt-4 space-y-3">
            {meal.FoodItems.map((foodItem) => (
              <li key={foodItem._id} className="flex justify-between items-center text-gray-700">
